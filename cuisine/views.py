...
@require_module_access('cuisine')
def etat_stock_date_cuisine_excel(request):
    """Export Excel : état du stock sur une période — Cuisine"""
    import openpyxl
    from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
    from openpyxl.utils import get_column_letter
    from django.http import HttpResponse
    ...
    ws['A1'] = "ÉTAT DU STOCK SUR PÉRIODE — CUISINE BEHANIAN"
    ...
    ws.append([])
    headers = ['#', 'Ingrédient', 'Catégorie', 'Unité',
               'Stock début', 'Entrées', 'Sorties', 'Casses', 'Productions', 'Inventaires',
               'Stock fin', 'Val. début (F)', 'Val. fin (F)']
    ws.append(headers)
    rh = ws.max_row
    ...
    for i, r in enumerate(resultats, 1):
        ...
        ws.append([
            i, ing.nom,
            ing.categorie.nom if ing.categorie else '—',
            str(ing.unite_stock) if ing.unite_stock else '—',
            float(r['stock_debut']), float(r['nb_entrees']), float(r['nb_sorties']),
            float(r['nb_casses']), float(r['nb_productions']), float(r['nb_inventaires']),
            float(r['stock_fin']), float(r['valeur_debut']), float(r['valeur_fin']),
        ])
        rw = ws.max_row
        for col in range(1, NC + 1):
            c = ws.cell(row=rw, column=col)
            c.font = bf if col in (2, 11, 12, 13) else nf
            c.border = bd
            c.alignment = rt if col >= 5 else (ct if col == 1 else Alignment(vertical='center'))
        ws.cell(row=rw, column=12).number_format = '#,##0'
        ws.cell(row=rw, column=13).number_format = '#,##0'

    ws.append([])
    tr = ws.max_row + 1
    ws.cell(row=tr, column=11, value='TOTAL').font = Font(name='Calibri', bold=True)
    ws.cell(row=tr, column=12, value=float(valeur_totale_debut)).font = Font(name='Calibri', bold=True, size=11)
    ws.cell(row=tr, column=12).number_format = '#,##0'
    ws.cell(row=tr, column=13, value=float(valeur_totale_fin)).font = Font(name='Calibri', bold=True, size=11, color='16a34a')
    ws.cell(row=tr, column=13).number_format = '#,##0'

    for col, w in enumerate([5, 28, 16, 9, 11, 9, 9, 9, 11, 11, 11, 16, 16], 1):
        ws.column_dimensions[get_column_letter(col)].width = w

    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    sfx = f"{date_debut.strftime('%Y%m%d')}{'_' + date_fin.strftime('%Y%m%d') if date_debut != date_fin else ''}"
    response['Content-Disposition'] = f'attachment; filename="Stock_Cuisine_{sfx}.xlsx"'
    wb.save(response)
    return response
...
def mouvements_print_cuisine(request):
    ...
    context = {
        ...
        'categories': CategorieIngredient.objects.all(),
        'ingredients_all': Ingredient.objects.filter(statut=True).order_by('nom'),
        'categorie_id': categorie_id,
        'ingredient_id': ingredient_id,
        'type_mv': type_mv,
        'nb_entrees': nb_entrees,
...
@require_module_access('cuisine')
def mouvements_excel_cuisine(request):
    """Export Excel : mouvements de stock — Cuisine"""
    import openpyxl
    from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
    from openpyxl.utils import get_column_letter
    from django.http import HttpResponse
    ...
    NC = 10
    ws.merge_cells(f'A1:{get_column_letter(NC)}1')
    ws['A1'] = "MOUVEMENTS DE STOCK — CUISINE BEHANIAN"
    ws['A1'].font = tf; ws['A1'].alignment = ct
    ...
    ws.append([])
    headers = ['#', 'Date', 'Heure', 'Ingrédient', 'Catégorie', 'Type', 'Quantité', 'Unité', 'Utilisateur', 'Commentaire']
    ws.append(headers)
    rh = ws.max_row
    for col in range(1, NC + 1):
        c = ws.cell(row=rh, column=col)
    ...
    for i, m in enumerate(mvts_list, 1):
        ...
        ws.append([
            ...
            m.date.strftime('%H:%M'),
            ing.nom if ing else '—',
            ing.categorie.nom if ing and ing.categorie else '—',
            TYPE_LABELS.get(m.type_mouvement, m.type_mouvement),
            float(m.quantite),
            str(ing.unite_stock) if ing and ing.unite_stock else '—',
        ...
        for col in range(1, NC + 1):
            ...
            c.border = bd
            c.fill = row_fill
            c.alignment = rt if col == 7 else (ct if col in (1, 2, 3, 8) else Alignment(vertical='center'))
    ...
    for col, w in enumerate([5, 12, 8, 28, 16, 16, 10, 9, 18, 30], 1):
        ws.column_dimensions[get_column_letter(col)].width = w

    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    sfx = timezone.now().strftime('%Y%m%d_%H%M')
    response['Content-Disposition'] = f'attachment; filename="Mouvements_Cuisine_{sfx}.xlsx"'
    wb.save(response)
    return response

...
def stats_ventes(request):
    """Page Statistiques Ventes — Top plats et top boissons."""
    from utils.permissions import _is_manager
    from django.contrib import messages as _msg
    if not (_is_manager(request.user) or request.user.is_superuser):
        ...
        return redirect('dashboard:index')
    today = timezone.now().date()
    return render(request, 'dashboard/stats_ventes.html', {
        'date_debut_default': (today - timedelta(days=29)).strftime('%Y-%m-%d'),
        'date_fin_default':   today.strftime('%Y-%m-%d'),
    })


@login_required
def api_stats_ventes(request):
    """JSON : top plats et top boissons sur une période."""
    from utils.permissions import _is_manager
    from datetime import datetime
    ...
    top_plats = [
        {
            'nom':      r['plat__nom'],
            'categorie': r['plat__categorie__nom'] or '—',
            'qte':      r['total_qte'],
            'ca':       int(r['total_ca'] or 0),
        }
        for r in qs_plats
    ]

    # ── Top Boissons : restaurant + cave TPE fusionnés ────────────────
    boissons: dict = {}
    ...
    for r in (
        ...
        boissons[bid] = {
            'nom':      r['boisson__nom'],
            'categorie': r['boisson__categorie__nom'] or '—',
            'qte':      r['total_qte'],
            'ca':       int(r['total_ca'] or 0),
        }
